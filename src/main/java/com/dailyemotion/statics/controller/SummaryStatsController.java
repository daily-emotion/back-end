...
import com.dailyemotion.statics.service.SummaryStatsService;
...
public class SummaryStatsController {
    ...
    @Operation(summary = "월별 감정/태그 통계 요약 조회",
            description = "특정 연도와 월의 가장 많이 사용된 감정과 태그를 조회합니다.")
    @ApiResponses({
    ...
    public ResponseEntity<SummaryStatsRes> getMonthlySummary(
            @Parameter(description = "조회할 연도", example = "2024", required = true)
            @PathVariable int year,
            @Parameter(description = "조회할 월(1-12)", example = "1", required = true)
            @PathVariable int month) {

        return ResponseEntity.ok(summaryStatsService.getMonthlySummary(year, month));
    }
}